...
<body>
    ...
    <div class="collection-container">
        ...
        <div class="collection-body-header">
            <h2 class="body-title" style="color: gray; font-weight:500">Collection</h2>
            <div class="collection-dropdown">
                <button onclick="dropdown()" class="collection-dropbtn">Category</button>
                <div id="myDropdown" class="collection-dropdown-content">
                    <a href="men.php">Men</a>
                    <a href="women.php">Women</a>
                </div>
            </div>

            <div class="collection-sell-button" onclick="sell()">
                <button class="collection-sell-item">Sell Item</button>
            </div>
        </div>
    </div>

    <div class="collection-container">
        <h3 class="collection-title">Your items</h3>
            ...
            ?>
                <div class="collection-product-section">
                    ...
                    <div class="collection-details">
                        <div class="collection-product-title"><?php echo $row['name'] ?></div>
                        <div class="collection-desc"><?php echo $row['description'] ?></div>
                        <div class="collection-quantity">Stocks: <?php echo $row['quantity'] ?></div>
                        <div class="collection-function">
                            <div class="collection-edit"><a href="editItem.php?id=<?php echo $row['id']; ?>">Edit</a></div>
                            <div class="collection-remove"><a href="deleteItem.php?id=<?php echo $row['id']; ?>">Remove</a></div>
                        </div>
                    </div>
                </div>
    ...
    <div class="collection-container">
        <h3 class="collection-title">Other User's item</h3>
        <div class="collection-body-content">
            <?php
            $rows = mysqli_query($conn, "SELECT * FROM collection WHERE NOT user_id = $user AND quantity != 0 ORDER BY id DESC");
            foreach ($rows as $row) : $explodeFiles = explode(" ", $row['image_path']);
            ?>
                <div class="collection-product-section">
                    ...
                    <div class="collection-details">
                        <div class="collection-product-title"><a href="index.php?id=<?php echo $row['id']; ?>"><?php echo $row['name'] ?></a></div>
                        <div class="collection-desc"><?php echo $row['description'] ?></div>
                        <div class="collection-quantity">Stocks: <?php echo $row['quantity'] ?></div>
                        <div class="collection-view"><a href="index.php?id=<?php echo $row['id']; ?>">View</a></div>
                    </div>
                </div>
            <?php endforeach ?>
        </div>
    ...
    <script>
        function dropdown() {
            document.getElementById("myDropdown").classList.toggle("collection-show");
        }
        window.onclick = function(event) {
            if (!event.target.matches('.collection-dropbtn')) {
                ...
                for (i = 0; i < dropdowns.length; i++) {
                    ...
                    if (openDropdown.classList.contains('collection-show')) {
                        openDropdown.classList.remove('collection-show');
                    }
                }
            }
        }
