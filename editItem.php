...
$result = mysqli_query($conn, "SELECT * FROM collection WHERE id='" . $_GET['id'] . "'");
...
if (isset($_POST["submit"])) {
    ...
    if (count($fileName) > 4) {
        echo "<script>alert('Files can be uploaded is limited to 4')</script>";
        ...
        $sql = "UPDATE collection " . "SET name = '$name', description = '$description', price = '$price', quantity = '$quantity', gender = '$gender', image_path = '$implodeFiles'" . "WHERE id = '$id'";
        ...
        echo "<script>alert('Item has been successfully updated!');
...
<body>
    ...
    <!-- HEAD CONTAINER -->
    <div class="container">
        <h2 class="body-title" style="color: gray; font-weight:100">Edit Item</h2>
        <form method="post" enctype="multipart/form-data">
            <input type="hidden" value="<?php echo $id; ?>">
            <div class="form-content1">
                <span class="form-titles">Sneaker name</span><br>
                <textarea rows="2" type="text" class="name" name="name" required><?php echo $results['name'] ?></textarea>
            </div>
            <div class="form-content2">
                <span class="form-titles">Description</span><br>
                <textarea rows="5" class="description" name="description" required><?php echo $results['description'] ?></textarea>
            </div>
            <div class="form-content3">
                <div class="gendersec">
                    <span class="form-titles">Price</span><br>
                    <input type="number" class="price" name="price" placeholder="USD" value="<?php echo $results['price'] ?>" required>
                </div>
                <div class="stocksec">
                    <span class="form-titles">Quantity</span><br>
                    <input type="number" class="quantity" name="quantity" value="<?php echo $results['quantity'] ?>" required>
                </div>
            </div>
            ...
            <div class="form-content5">
                ...
                <input type="file" class="image_path" name="image_path[]" accept=".jpg, .jpeg, .png" value="<?php echo $results['image_path']; ?>" multiple required>
            </div>
            <button type="submit" class="sell-submit" value="submit" name="submit">Submit</button>
        </form>
    </div>
